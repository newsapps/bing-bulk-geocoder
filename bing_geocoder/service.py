...
if boto is not None:
    def download_jobs(geocoder):
        ...
        files = bucket.list(awaiting_folder)
        for f in files:
            try:
                name = f.name.replace(awaiting_folder + '/', '')
                fkey = bucket.get_key(f.name)
                email_address = fkey.get_metadata('email')
                if name:
                    logging.info("Uploading {} to Bing".format(name))
                    job_id = geocoder.upload_address_batch(fkey.get_contents_as_string())
                    if job_id:
                        logging.info("Moving batch with old id {} to new id {} in {}".format(
                            name, job_id, pending_folder))
                        new_key = Key(bucket)
                        new_key.key = '{}/{}'.format(pending_folder, job_id)
                        if email_address:
                            logging.info("Setting metadata to " + email_address)
                        ...
                        old_key = Key(bucket)
                        old_key.key = '{}/{}'.format(awaiting_folder, name)
                        old_key.delete()
                    else:
                        send_email_notification(email_address, {}, name, 'error')

            except Exception as e:
                logging.warning("Error uploading {} to Bing: {}".format(name, e))
    ...
    def check_pending_jobs(geocoder):
        ...
        connection = boto.connect_s3()
        bucket = connection.get_bucket(GEO_BUCKET)
        files = bucket.list(pending_folder)
        for f in files:
            job_id = f.name.replace(pending_folder + '/', '')
            if job_id:
                results = geocoder.get_job_statuses(job_id=job_id)
                if results[0]['status'] == 'Completed':
                    k = bucket.get_key('{}/{}'.format(pending_folder, job_id))
...
if sendgrid:
    def send_email_notification(address, results, settings, status):
        ...
        """
        finished_url = 'http://geo.tribapps.com/geocode_finished_jobs/' + settings
        if status == 'finished':
            if not results:
                results = [{}]
            subject = 'Finished geocoding job ' + settings
            template = """
            ...
            subject = 'Began processing geocode job ' + settings
            template = """
            Just submitted job for geocoding.<br><br>

            We'll email you when it's done, but the results will be available at 
            """ + finished_url
        elif status == 'error':
            subject = 'Error processing geocode job ' + settings
            template = """
        ...
        logging.info('Sending email report to ' + ', '.join(message.to))
        status, msg = sg.send(message)
        if status != 200:
            logging.warning('Error sending email, got status {}: {}'.format(status, msg))
